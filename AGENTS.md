...
9. **筹码与机构证据**（2026-06-15）：`stock_intelligence_refresh.py` 在收盘后刷新全部持仓和
   动态候选前5名，统一采集解禁、两融、股东户数、龙虎榜、大宗交易和研报 EPS 预期，写入
   `market_snapshot` 与 `stock-triage/cache/market_intelligence/`。竞价/开盘只读缓存；
   30日内解禁比例达到10%进入 Policy 硬门禁。缓存过期或端点缺失必须披露，不得把空数据
   当作“无风险”。解禁/两融/股东户数任一必要数据缺失或过期时只能输出关注；仅可回退到
   仍在有效期内的最近可信快照。东方财富调用必须走统一 adapter 的严格响应校验、共享限速
   和熔断，禁止业务脚本自行 `request_json` 或 `curl`。Serenity 只能把研报和一致预期作为
   B 级佐证，公告和财报仍是事实源。
