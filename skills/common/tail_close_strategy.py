...
def validate_prepared_state(
    ...
    prepared_payload = {key: value for key, value in prepared_state.items() if key != "prepared_hash"}
    ...
    if str(bundle.get("prepare_run_id") or "").strip() != prepared_identity["run_id"]:
...
def validate_pit_bundle(
    ...
    *,
    ...
    sealed_at = _validate_pit_identity(bundle, cutoff)
    maximum_event, maximum_available = _validate_pit_records(bundle, cutoff)
    _validate_minute_row_dates(bundle, trading_date)
    freshness_floor = cutoff - timedelta(seconds=int(maximum_current_record_age_seconds))
    _validate_current_record_freshness(bundle, freshness_floor)
    watermarks = _validate_source_watermarks(
        bundle,
        cutoff,
        freshness_floor,
        maximum_event,
...
def _validate_pit_records(
    ...
    maximum_event: datetime | None = None
    ...
    return maximum_event, maximum_available


def _validate_minute_row_dates(
    bundle: Mapping[str, Any],
    trading_date: str,
) -> None:
    for collection in ("sectors", "stocks"):
        for item in bundle.get(collection) or []:
            ...
            for child_collection in ("minute_rows", "bars"):
                for child in item.get(child_collection) or []:
                    ...
                    if event.date().isoformat() != trading_date or available.date().isoformat() != trading_date:
                        raise TailCloseContractError("minute_row_trading_date_mismatch")


def _validate_current_record_freshness(
    bundle: Mapping[str, Any],
...
def is_main_board_code(code: Any) -> bool:
    ...
    return (
        ...
        and normalized.startswith(("000", "001", "002", "003", "600", "601", "603", "605"))
    )
...
def _universe_reasons(stock: Mapping[str, Any], config: Mapping[str, Any]) -> list[str]:
    ...
    elif listing_days < float(config["universe"]["minimum_listing_trading_days"]):
    ...
    elif median_amount < float(config["universe"]["minimum_median_turnover_20d_cny"]):
    ...
    elif pit_amount < float(config["universe"]["minimum_turnover_at_cutoff_cny"]):
...
def build_prepared_state(
    ...
    pit = validate_pit_bundle(
        ...
        maximum_clock_offset_seconds=int(config["runtime"]["maximum_clock_offset_seconds"]),
        maximum_current_record_age_seconds=int(config["runtime"]["prepare_maximum_current_record_age_seconds"]),
    )
...
def _percentile_ranks(values: Mapping[str, float]) -> dict[str, float]:
    ...
    percentile = {value: index / (len(unique) - 1) for index, value in enumerate(unique)}
...
def rank_sectors(
    ...
    for raw in sectors:
        ...
        required = {
            "session_relative_return": _as_float(sector.get("session_relative_return", sector.get("day_excess"))),
            "tail_relative_return": _as_float(sector.get("tail_relative_return", sector.get("tail_excess"))),
            "breadth": breadth,
        ...
        if (
            ...
            or member_count < int(config["sector_gate"]["minimum_valid_constituents"])
            or breadth is None
    ...
    for sector in eligible:
        ...
        score = sum(float(weights[name]) * component_ranks[name][sector_id] for name in component_ranks)
        ranked.append(
            {
                **sector,
                "mainline_score": round(score, 8),
                "component_ranks": {name: round(component_ranks[name][sector_id], 8) for name in component_ranks},
            }
        )
    ranked.sort(
        key=lambda item: (
    ...
    keep = (
        max(
            1,
            math.ceil(len(ranked) * float(config["sector_gate"]["top_cross_section_fraction"])),
        )
        if ranked
        else 0
    )
...
def _tail_minute_rows(stock: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    return [
        row
...
def _price_structure_reasons(
    ...
    if not float(gate["minimum_session_return"]) <= session_return <= float(gate["maximum_session_return"]):
    ...
    if not (float(fields["ma5"]) > float(fields["ma10"]) >= float(fields["ma20"]) and price > float(fields["ma20"])):
...
def _tail_pattern_reasons(
    ...
    reasons: list[str] = []
    above = sum(
        1 for minute_price, minute_vwap in zip(prices, vwaps) if float(minute_price) > float(minute_vwap)
    ) / len(prices)
    ...
    positive_steps = [max(0.0, float(current) - float(previous)) for previous, current in zip(prices, prices[1:])]
    ...
    for (previous_price, _), (current_price, current_volume) in zip(
        ...
        if minute_return <= float(gate["maximum_single_minute_loss"]) and float(
            current_volume
...
def _stock_gate_reasons(
    ...
    session_return = _as_float(stock.get("session_return", stock.get("day_change_pct")))
    if session_return is None:
        reasons.append("session_return_missing")
        return list(dict.fromkeys(reasons))
    vwap = float(fields["vwap"])
    gate = config["stock_gate"]
    reasons.extend(_price_structure_reasons(stock, fields, session_return, gate))
    rows = _tail_minute_rows(stock)
    ...
    if any(value is None or value <= 0 for value in volumes):
        ...
        return list(dict.fromkeys(reasons))
    reasons.extend(_tail_pattern_reasons(prices, vwaps, volumes, gate))
    return list(dict.fromkeys(reasons))
...
def _rankable_stock(
    stock: Mapping[str, Any],
    code: str,
    sector_score: Mapping[str, float],
    config: Mapping[str, Any],
) -> dict[str, Any]:
    rows = _tail_minute_rows(stock)
    prices = [float(row.get("close", row.get("price"))) for row in rows]
    volumes = [float(row.get("volume") or 0) for row in rows]
    continuity = sum(1 for previous, current in zip(prices, prices[1:]) if current >= previous) / max(
        1, len(prices) - 1
    )
    volume_quality = 1.0 - (max(volumes) / sum(volumes) if sum(volumes) > 0 else 1.0)
    minimum_turnover = float(config["universe"]["minimum_turnover_at_cutoff_cny"])
    execution_quality = min(
        1.0,
        float(stock.get("amount") or 0) / max(minimum_turnover, 1.0),
    )
    return {
        **stock,
        "code": code,
...
def _rank_eligible_stocks(
    ...
    components = {
        name: _percentile_ranks({item["code"]: float(item[field]) for item in eligible})
    ...
    for item in eligible:
        ...
        public["rank_components"] = {name: round(components[name][code], 8) for name in components}
...
def _select_distinct_sectors(
    ...
    for item in ranked:
        ...
        if len(selected) >= maximum_signals:
            break
    return selected
...
def rank_stocks(
    ...
    qualified = {str(item["sector_id"]) for item in ranked_sectors if item.get("mainline_qualified")}
    sector_score = {str(item["sector_id"]): float(item["mainline_score"]) for item in ranked_sectors}
    eligible: list[dict[str, Any]] = []
    rejected: list[dict[str, Any]] = []
    for raw in stocks:
        stock = dict(raw)
        ...
        if reasons:
            ...
            continue
        eligible.append(_rankable_stock(stock, code, sector_score, config))
    ranked = _rank_eligible_stocks(eligible, config)
    selected = _select_distinct_sectors(
        ranked,
        int(config["ranking"]["maximum_research_signals"]),
    )
    return {
        "ranked_candidates": ranked[: int(config["ranking"]["maximum_research_candidates"])],
        "selected": selected,
        "rejected": rejected,
    }


def _finalize_decision(result: dict[str, Any]) -> dict[str, Any]:
    result["decision_hash"] = canonical_hash(result)
    return result


def _decision_window_result(
    *,
    trading_date: str,
    status: str,
    reason: str,
...
def _market_gate_reasons(
    ...
    if market.get("regime") in {"risk_off", "unknown", "stale"}:
        reasons.append(f"market_{market.get('regime')}")
    ...
    if benchmark_tail is None or benchmark_tail < float(
        ...
        reasons.append("benchmark_tail_weak")
    if breadth is None or breadth < float(config["market_gate"]["minimum_advancer_ratio"]):
        reasons.append("market_breadth_weak")
    return reasons


def _market_rejection_result(
    trading_date: str,
    ...
    return _finalize_decision(
        {
            ...
            "status": "no_action",
            "signals": [],
            "market_gate": {"allowed": False, "reasons": list(reasons)},
            "pit": pit,
            "research_only": True,
            "live_weight": 0.0,
            "automatic_order_count": 0,
...
def _prepared_decision_inputs(
    stocks: Sequence[Mapping[str, Any]],
    prepared_state: Mapping[str, Any],
) -> tuple[list[Mapping[str, Any]], list[dict[str, Any]]]:
    allowed_codes = {str(item.get("code") or "").zfill(6) for item in prepared_state.get("eligible") or []}
    rejections = [
        {
    ...
    decision_stocks = [item for item in stocks if str(item.get("code") or "").zfill(6) in allowed_codes]
    return decision_stocks, rejections


def _build_research_signal(
    ...
    prepared_identity: Mapping[str, str],
) -> dict[str, Any]:
    config_hash = canonical_hash(config)
    bundle_hash = canonical_hash(bundle)
    signal_id = (
        f"tail-{trading_date.replace('-', '')}-{item['code']}-"
        f"{canonical_hash({'bundle': bundle_hash, 'config': config_hash})[:10]}"
    )
    ...
    return {
        ...
        "requested_capacity": config["execution"]["research_notional_per_signal_cny"],
        "proposed_position_pct": (
            float(config["execution"]["research_notional_per_signal_cny"])
            / float(config["portfolio"]["research_portfolio_notional_cny"])
            * 100
        ),
        "requested_action": "buy",
        "decision_mode": decision_mode,
        "snapshot_id": bundle.get("snapshot_id"),
        "snapshot_hash": bundle.get("snapshot_hash") or bundle_hash,
...
def _successful_decision_result(
    *,
    ...
    pit: Mapping[str, Any],
    sectors: Sequence[Mapping[str, Any]],
    stock_result: Mapping[str, Any],
    prepared_rejections: Sequence[Mapping[str, Any]],
...
def build_research_decision(
    ...
    *,
    ...
    pit = validate_pit_bundle(
        ...
        maximum_clock_offset_seconds=int(config["runtime"]["maximum_clock_offset_seconds"]),
        maximum_current_record_age_seconds=int(config["runtime"]["decision_maximum_current_record_age_seconds"]),
    )
    ...
    if emitted < decision_at:
        return _decision_window_result(
            trading_date=trading_date,
    ...
    market_reasons = _market_gate_reasons(market, config)
    if market_reasons:
        return _market_rejection_result(trading_date, market_reasons, pit)

    sectors = rank_sectors(bundle.get("sectors") or [], config)
    decision_stocks, prepared_rejections = _prepared_decision_inputs(
        list(bundle.get("stocks") or []),
        prepared_state,
    )
    stock_result = rank_stocks(decision_stocks, sectors, config)
    signals = [
        _build_research_signal(
            item,
            index,
            trading_date,
            bundle,
    ...
    return _successful_decision_result(
        trading_date=trading_date,
        emitted=emitted,
        pit=pit,
        sectors=sectors,
...
def _continuous_fill_context(
    ...
    *,
    ...
    if signal.get("research_only") is not True or float(signal.get("live_weight") or 0) != 0:
    ...
    trading_date = str(signal.get("trading_date") or "") or str(decision_emitted_at)[:10]
    ...
    limit_price = reference * (1 + float(config["execution"]["maximum_limit_premium"]))
    ...
    return {
        ...
        "participation": float(config["execution"]["maximum_visible_volume_participation"]),
        "queue_discount": float(config["execution"]["queue_discount"]),
    }


def _continuous_bar_candidates(
    bars: Sequence[Mapping[str, Any]],
    context: Mapping[str, Any],
) -> list[tuple[datetime, datetime, float, int]]:
    trading_date = str(context["trading_date"])
    arrival = context["arrival"]
    cancel = context["cancel"]
    limit_price = float(context["limit_price"])
    participation = float(context["participation"])
    queue_discount = float(context["queue_discount"])
    eligible: list[tuple[datetime, datetime, float, int]] = []
    for row in bars:
        ...
        if price is None or price <= 0 or price > limit_price:
            continue
        visible = _as_float(row.get("available_sell_volume", row.get("sell_volume", row.get("volume"))))
...
def _consume_continuous_candidates(
    eligible: Sequence[tuple[datetime, datetime, float, int]],
...
def _continuous_fill_result(
    ...
    requested_quantity = int(context["requested_quantity"])
    filled_quantity = requested_quantity - remaining
    fill_price = sum(item["price"] * item["quantity"] for item in fills) / filled_quantity if filled_quantity else None
    ...
    result = {
        ...
        "status": status,
        "requested_notional": round(float(context["notional"]), 4),
        "portfolio_allocated_capacity": round(
            float(context["allocated_capacity"]),
            4,
        ),
        "requested_quantity": requested_quantity,
        "filled_quantity": filled_quantity,
        "unfilled_quantity": remaining,
        "reference_price": context["reference"],
        "limit_price": round(float(context["limit_price"]), 4),
        "fill_price": round(fill_price, 4) if fill_price else None,
        "fills": list(fills),
        "simulated_arrival_time": context["arrival"].isoformat(),
        "simulated_cancel_time": context["cancel"].isoformat(),
        "buy_cost": buy_cost,
        "simulation": True,
        "broker_called": False,
        "research_only": True,
...
def simulate_continuous_fill(
    ...
    *,
    ...
    context = _continuous_fill_context(
        signal,
        config,
        decision_emitted_at=decision_emitted_at,
        requested_notional=requested_notional,
    )
    eligible = _continuous_bar_candidates(bars, context)
    remaining, fills = _consume_continuous_candidates(
        eligible,
        int(context["requested_quantity"]),
    )
    return _continuous_fill_result(signal, config, context, remaining, fills)


def _exit_bar_candidates(
    ...
    for row in session.get("bars") or []:
        ...
        if event_at.date().isoformat() != day or available_at.date().isoformat() != day:
        ...
        if not (window_start_at <= event_at < window_end_at and available_at <= window_end_at):
            continue
        if row.get("blocked") is True:
            continue
        price = _as_float(row.get("bid_price", row.get("price", row.get("close"))))
        volume = _as_float(row.get("available_buy_volume", row.get("buy_volume", row.get("volume"))))
        if price and volume and volume > 0:
            executable.append((event_at, available_at, float(price), int(volume / 100) * 100))
    executable.sort(key=lambda item: (item[0], item[1], item[2]))
    return executable


def _consume_exit_bars(
    executable: Sequence[tuple[datetime, datetime, float, int]],
    remaining: int,
    day: str,
    exit_fills: list[dict[str, Any]],
) -> int:
    for index, (event_at, available_at, price, available) in enumerate(executable):
...
def _collect_exit_observations(
    sessions: Sequence[Mapping[str, Any]],
    allowed_days: set[str],
    window_start: str,
    window_end: str,
    quantity: int,
) -> tuple[int, list[dict[str, Any]], list[float], int]:
    remaining = quantity
    exit_fills: list[dict[str, Any]] = []
    marks: list[float] = []
    blocked_days = 0
    eligible_sessions = (
        item for item in sessions if isinstance(item, Mapping) and str(item.get("trading_date") or "") in allowed_days
    )
    for session in sorted(
        ...
        executable = _exit_bar_candidates(session, day, window_start, window_end)
        ...
        if not executable:
            blocked_days += 1
            continue
        remaining = _consume_exit_bars(executable, remaining, day, exit_fills)
        if remaining <= 0:
            break
        blocked_days += 1
    return remaining, exit_fills, marks, blocked_days


def _estimate_outcome_pnl(
    *,
    entry_price: float,
    exit_price: float | None,
    quantity: int,
    remaining: int,
    exit_fills: Sequence[Mapping[str, Any]],
    conservative_mark: float,
    entry_day: str,
    ...
    if remaining == 0 and exit_price:
        return estimate_round_trip_pnl(
    ...
    realised_exit_value = sum(item["price"] * item["quantity"] for item in exit_fills)
    ...
    return {
        ...
        "estimated_net_pnl": round(exit_value - entry_value - estimated_cost, 4),
...
def _build_outcome_result(
    ...
    *,
    ...
    return {
        ...
        "status": ("exited" if remaining == 0 else "right_censored" if right_censored else "blocked_pending"),
        ...
        "exited_quantity": quantity - remaining,
...
def _observation_complete(
    sessions: Sequence[Mapping[str, Any]],
    allowed_day_sequence: Sequence[str],
) -> bool:
    allowed_days = set(allowed_day_sequence)
    observed_dates = {
        str(item.get("trading_date") or "")
        for item in sessions
        if isinstance(item, Mapping) and str(item.get("trading_date") or "") in allowed_days
    }
    return allowed_days.issubset(observed_dates)


def _corporate_action_status(
    sessions: Sequence[Mapping[str, Any]],
) -> str:
    if all(str(item.get("corporate_action_status") or "clear") == "clear" for item in sessions):
        return "clear"
...
def label_d1_outcome(
    ...
    allowed_day_sequence = [add_trading_days(entry_day, offset).isoformat() for offset in range(1, maximum_days + 1)]
    allowed_days = set(allowed_day_sequence)
    remaining, exit_fills, marks, blocked_days = _collect_exit_observations(
        sessions,
        allowed_days,
        window_start,
        window_end,
    ...
    exit_price = sum(item["price"] * item["quantity"] for item in exit_fills) / exited if exited else None
    observation_complete = _observation_complete(sessions, allowed_day_sequence)
    right_censored = remaining > 0 and observation_complete
    conservative_mark = min(marks) if marks else entry_price
    corporate_action_status = _corporate_action_status(sessions)
    pnl = _estimate_outcome_pnl(
        entry_price=entry_price,
    ...
    tail_loss_pct = min((mark / entry_price - 1) * 100 for mark in marks) if marks and entry_price > 0 else None
...
def simulate_after_hours_fixed_fill(
    ...
    for row in observations:
        ...
        if decision <= event <= end and observed >= event and observed <= end:
            if "incremental_matched_sell_volume" not in row:
                raise TailCloseContractError("after_hours_incremental_queue_volume_missing")
            available += max(
                0,
    ...
    filled = min(
        quantity,
        int(available * float(config["execution"]["after_hours_queue_discount"]) / 100) * 100,
    )
    status = "FULL_FILL" if filled == quantity else ("PARTIAL_FILL" if filled else "UNFILLED")
    return {
        "schema": AFTER_HOURS_SCHEMA,
        "strategy_id": AFTER_HOURS_STRATEGY_ID,
        "status": status,
...
def evaluate_kill_switch(
    ...
    if fill_error is not None and fill_error > float(config["validation"]["shadow"]["maximum_fill_rate_error"]):
    ...
    return {
        ...
        "affected_strategy_id": ("*" if global_block else metrics.get("strategy_id")),
