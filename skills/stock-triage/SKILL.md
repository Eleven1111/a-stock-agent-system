...
本技能是决策编排层。需要「原始事实」或「研究性论证」时都不该从这里进入。

| 场景 | 改用 | 原因 |
| --- | --- | --- |
| 要原始行情、K 线、财务数据 | `a-stock-data` / `stock-analyst` | 本技能不是数据源，只消费同一运行批次已产出的证据 |
| 要涨停梯队、板块赚钱效应、情绪温度 | `hot-money-tactics` | 本技能不做全市场情绪聚合 |
| 要竞价/开盘的候选判定与六问否决 | `daban-stock-picker` | 本技能不做单候选的可成交性判定 |
| 要多专家论证、红队反证、深度研究 | `research-committee` | 本技能只执行 Policy，不做研究性论证 |
| 要真实下单或撤单 | 无 | 仅决策支持，任何情况下不自动下单 |

另有两种误用：

- **不要从聊天历史恢复持仓或监控。** 持仓、监控、候选一律从运行时状态读取；用户口述的持仓必须先写入再参与决策。
- **不要在证据缺失时要求给方向。** 数据缺失、过期、来源失败或交易日历未覆盖时，本技能只输出观察条件与阻断原因，不会给出无条件买入。
