...
- 09:50 优先复用 09:35 候选；09:35 降级时从 `evaluated_confirmations`、
  `confirmations` 或同日竞价短名单恢复。09:50 和 13:15 均最多处理 20 只候选，
  HTTPS 快照后有界补取分钟线，且计算前按各自时间窗截断。
...
| 09:29 | 风险预计算 | 前日及更早日线生成组合因子与流动性证据，缺失继续阻断 |
| 09:35 | 早盘确认 | 5 分钟观察覆盖、公告质检、可成交性、板块内开盘排名、策略门禁；不伪造 15/30 分钟指标 |
| 09:50 | 承接复核 | `confirmed/watch/invalidated` 研究状态 |
| 13:15 | 午后回流 | `confirmed/watch/invalidated` 研究状态 |

09:50/13:15 不产生订单，不新增推荐记录，也不建议当日卖出。每条记录固定包含 `execution_action=none`、`same_day_sell_allowed=false` 和下一交易日最早卖出日期。09:50 的恢复路径仍是 research-only，不会把趋势 lane 的实时权重从零改为非零。
